...
class advancedMemAgent:
    def __init__(self, model, backend, retrieve_k, temperature_c5, sglang_host="http://localhost", sglang_port=30000, api_key=None, api_base=None):
...
def evaluate_dataset(dataset_path: str, model: str, output_path: Optional[str] = None, ratio: float = 1.0, backend: str = "sglang", temperature_c5: float = 0.5, retrieve_k: int = 10, sglang_host: str = "http://localhost", sglang_port: int = 30000, api_key: Optional[str] = None, api_base: Optional[str] = None):
    ...
    for sample_idx, sample in enumerate(samples):
        agent = advancedMemAgent(model, backend, retrieve_k, temperature_c5, sglang_host, sglang_port, api_key, api_base)
        ...
        for qa in sample.qa:
            if int(qa.category) in allow_categories:
                ...
                clean_context = raw_context.encode('ascii', 'ignore').decode('ascii')
                logger.info(f"Raw Context: {clean_context}")
...
def main():
    ...
    parser.add_argument("--api_base", type=str, default=None,
                      help="OpenAI API base URL (optional, uses OPENAI_BASE_URL env var if not provided)")
    ...
    evaluate_dataset(dataset_path, args.model, output_path, args.ratio, args.backend, args.temperature_c5, args.retrieve_k, args.sglang_host, args.sglang_port, args.api_key, args.api_base)
